---
title: "Platsannonser (hos Af) - Västernorrland"
author: "Detta är ett PM från Tillväxtverksamheten."
date: "`r format(Sys.Date(), '%Y-%m-%d')`"
output: 
  html_document:
    css: RVN-style_CF.css  
    toc: yes
    toc_float:
      toc_collapsed: true
    toc_depth: 3
...
df <- filter(df, substr(workplace_address.municipality_code,1,2) == 22)
...
## 2284
df_temp <- filter(df, substr(workplace_address.municipality_code,1,4) == 2284)
ovik <- sum(df_temp$number_of_vacancies, na.rm = T)
ovik <- format(ovik, big.mark=" ")

## 2283
df_temp <- filter(df, substr(workplace_address.municipality_code,1,4) == 2283)
solle <- sum(df_temp$number_of_vacancies, na.rm = T)
solle <- format(solle, big.mark=" ")

## 2282
df_temp <- filter(df, substr(workplace_address.municipality_code,1,4) == 2282)
kramfors <- sum(df_temp$number_of_vacancies, na.rm = T)
kramfors <- format(kramfors, big.mark=" ")

## 2280
df_temp <- filter(df, substr(workplace_address.municipality_code,1,4) == 2280)
hsd <- sum(df_temp$number_of_vacancies, na.rm = T)
hsd <- format(hsd, big.mark=" ")

## 2262
df_temp <- filter(df, substr(workplace_address.municipality_code,1,4) == 2262)
timra <- sum(df_temp$number_of_vacancies, na.rm = T)
timra <- format(timra, big.mark=" ")

## 2281
df_temp <- filter(df, substr(workplace_address.municipality_code,1,4) == 2281)
svall <- sum(df_temp$number_of_vacancies, na.rm = T)
svall <- format(svall, big.mark=" ")

## 2260
df_temp <- filter(df, substr(workplace_address.municipality_code,1,4) == 2260)
ange <- sum(df_temp$number_of_vacancies, na.rm = T)
ange <- format(ange, big.mark=" ")

```

* `r summa` lediga tjänster i `r geografi` hos Platsbanken, varav:
  + `r ovik` i Örnsköldsviks kommun
  + `r solle` i Sollefteå kommun
  + `r kramfors` i Kramfors kommun
  + `r hsd` i Härnösand kommun
  + `r timra` i Timrå kommun
  + `r svall` i Sundsvall kommun
  + `r ange` i Ånge kommun
...
color_mapping <- c("2260" = "#440154", "2281" = "#443A83", "2262" = "#31688E", "2280" = "#21908C", 
                   "2282" = "#35B779", "2283" = "#8FD744", "2284" = "#FDE725")

# VIZ
## ggplot
ggplot(df_22_kom, aes(x = occupation_field.label, y = total_vac, fill = factor(workplace_address.municipality_code, levels = c("2260", "2281", "2262", "2280", "2282","2283","2284")))) +
  geom_col() +
...
```
*Kommunkod: 2284 - Örnsköldsvik, 2283 - Sollefteå, 2282 - Kramfors, 2280 - Härnösand, 2262 - Timrå, 2281 - Sundsvall, 2260 - Ånge. För att underlätta läsning av diagrammet har 'klossar' införts - varje kloss motsvarar 50 tjänster.*
...
# INPUT
filter <- "Administration, ekonomi, juridik"
...
gg<-ggplot(df_22_omrade, aes(x = ssyk3_text, y = total_vac, fill = factor(workplace_address.municipality_code, levels = c("2260", "2281", "2262", "2280", "2282","2283","2284")))) +
  geom_col() +
  scale_fill_manual(values=color_mapping, guide = guide_legend(reverse = TRUE)) +
  #scale_fill_viridis(discrete = TRUE, direction = 1, guide = guide_legend(reverse = TRUE)) +
  theme_tufte() +
...
ggplot(df_22_omrade, aes(x = ssyk3_text, y = total_vac, fill = factor(workplace_address.municipality_code, levels = c("2260", "2281", "2262", "2280", "2282","2283","2284")))) +
  geom_col() +
  scale_fill_manual(values=color_mapping, guide = guide_legend(reverse = TRUE)) +
  #scale_fill_viridis(discrete = TRUE, direction = 1, guide = guide_legend(reverse = TRUE)) +
  geom_hline(yintercept = seq(from = 0, to = max_value, by = 50), color = "white") +
...
if (nrow(df_22_omrade_kval) > 0) {
  ...
  # No action is taken if the filtered dataframe is empty
}
...
# INPUT - occ.field
field_name <- "Militära yrken"

df_22_omrade_kval <- filter(df, occupation_field.label == field_name)
...
if (nrow(df_22_omrade_kval) > 0) {
  ...
  ggplot(df_22_omrade_kval, aes(x = ssyk3_text, y = total_vac, fill = factor(kvalifikations_niva, levels = c("4", "3", "2", "1")))) +
    ...
    theme(plot.title = element_text(size = 14, face = "bold"),
          ...
          axis.ticks.x = element_blank()) +
    labs(title = "Lediga tjänster",
         subtitle = sprintf("%s, SSYK3 - %s", field_name, geografi),
         y = "Tjänster",
         x = "",
         caption = "Källa: Af och bearbetningar av Region Västernorrland") +
    scale_y_continuous(labels = scales::number_format(accuracy = 1)) +
...
## Filter on 22
df_22_omrade <- filter(df, occupation_field.label == field_name)
...
## Filter
df_22_omrade_kval <- filter(df, occupation_field.label == "Naturvetenskap")
...
ggplot(df_22_omrade_kval, aes(x = ssyk3_text, y = total_vac, fill = factor(kvalifikations_niva, levels = c("4", "3", "2", "1")))) +
  ...
  theme(plot.title = element_text(size = 14, face = "bold"),
        ...
        axis.ticks.x = element_blank()) +
  labs(title = "Lediga tjänster",
       subtitle = sprintf("Naturvetenskap, SSYK3 - %s", geografi),
       y = "Tjänster",
       x = "",
       caption = "Källa: Af och bearbetningar av Region Västernorrland") +
  scale_y_continuous(labels = number_format(accuracy = 1))+
...
## Filter
df_22_omrade_kval <- filter(df, occupation_field.label == "Pedagogiskt arbete")
...
if (nrow(df_22_omrade_kval) == 0) {
  ...
  gg <- ggplot(df_22_omrade_kval, aes(x = ssyk3_text, y = total_vac, fill = factor(kvalifikations_niva, levels = c("4", "3", "2", "1")))) +
    ...
    theme(plot.title = element_text(size = 14, face = "bold"),
          ...
          axis.ticks.x = element_blank()) +
    labs(title = "Lediga tjänster",
         subtitle = sprintf("Pedagogiskt arbete, SSYK3 - %s", geografi),
         y = "Tjänster",
         x = "",
         caption = "Källa: Af och bearbetningar av Region Västernorrland") +
    scale_y_continuous(labels = number_format(accuracy = 1))+
...
## Filter on 22
df_22_omrade <- filter(df, occupation_field.label == "Pedagogiskt arbete")
...
ggplot(df_22_omrade_kval, aes(x = ssyk3_text, y = total_vac, fill = factor(kvalifikations_niva, levels = c("4", "3", "2", "1")))) +
  ...
  labs(title = "Lediga tjänster",
       ...
       caption = "Källa: Af och bearbetningar av Region Västernorrland") +
  coord_flip()+
  scale_y_continuous(labels = number_format(accuracy = 0))
...
gg<-ggplot(df_22_ssyk1_2_omrade, aes(x = occupation_field.label, y = total_vac, fill = factor(workplace_address.municipality_code, levels = c("2260", "2281", "2262", "2280", "2282","2283","2284")))) +
  geom_col() +
  scale_fill_manual(values=color_mapping, guide = guide_legend(reverse = TRUE)) +
  theme_tufte() +
  theme(plot.title = element_text(size = 14, face = "bold"),
...
# Check if the filtered dataframe is empty
if (nrow(df_22_omrade_ssyk1_2_Tssyk3) > 0) {
...
Kontakt: christian.forsberg@rvn.se

<br>

```{r, out.width = "20%", fig.align = "center", echo=FALSE}
knitr::include_graphics("rvn_top.jpg")
#20%
```