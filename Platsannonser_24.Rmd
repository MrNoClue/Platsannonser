---
title: "Platsannonser (hos Af) - Västerbotten"
...
:::{#preface}
<p>
Detta PM visar annonsdata från Arbetsförmedlingens JobTech API, till och med 2024-05-31 - syftet är att ge en nulägesbild av eftersökt kompetens (yrken).
...
relative_path <- "platsannonser.xlsx"
...
geografi<-"Västerbotten"

df <- filter(df, substr(workplace_address.municipality_code,1,2) == 24)
...
## 2284
df_temp <- filter(df, substr(workplace_address.municipality_code,1,4) == 2422)
sorsele <- sum(df_temp$number_of_vacancies, na.rm = T)
sorsele <- format(sorsele, big.mark=" ")

## 2283
df_temp <- filter(df, substr(workplace_address.municipality_code,1,4) == 2421)
storuman <- sum(df_temp$number_of_vacancies, na.rm = T)
storuman <- format(storuman, big.mark=" ")

## 2282
df_temp <- filter(df, substr(workplace_address.municipality_code,1,4) == 2482)
skelleftea <- sum(df_temp$number_of_vacancies, na.rm = T)
skelleftea <- format(skelleftea, big.mark=" ")

## 2280
df_temp <- filter(df, substr(workplace_address.municipality_code,1,4) == 2418)
mala <- sum(df_temp$number_of_vacancies, na.rm = T)
mala <- format(mala, big.mark=" ")

## 2281
df_temp <- filter(df, substr(workplace_address.municipality_code,1,4) == 2417)
norsjo <- sum(df_temp$number_of_vacancies, na.rm = T)
norsjo <- format(norsjo, big.mark=" ")

## 2260
df_temp <- filter(df, substr(workplace_address.municipality_code,1,4) == 2462)
vilhelmina <- sum(df_temp$number_of_vacancies, na.rm = T)
vilhelmina <- format(vilhelmina, big.mark=" ")

## 2281
df_temp <- filter(df, substr(workplace_address.municipality_code,1,4) == 2481)
lycksele <- sum(df_temp$number_of_vacancies, na.rm = T)
lycksele <- format(lycksele, big.mark=" ")

## 2260
df_temp <- filter(df, substr(workplace_address.municipality_code,1,4) == 2404)
vindeln <- sum(df_temp$number_of_vacancies, na.rm = T)
vindeln <- format(vindeln, big.mark=" ")


df_temp <- filter(df, substr(workplace_address.municipality_code,1,4) == 2425)
dorotea <- sum(df_temp$number_of_vacancies, na.rm = T)
dorotea <- format(dorotea, big.mark=" ")


df_temp <- filter(df, substr(workplace_address.municipality_code,1,4) == 2409)
robertsfors <- sum(df_temp$number_of_vacancies, na.rm = T)
robertsfors <- format(robertsfors, big.mark=" ")


df_temp <- filter(df, substr(workplace_address.municipality_code,1,4) == 2480)
umea <- sum(df_temp$number_of_vacancies, na.rm = T)
umea <- format(umea, big.mark=" ")


df_temp <- filter(df, substr(workplace_address.municipality_code,1,4) == 2463)
asele <- sum(df_temp$number_of_vacancies, na.rm = T)
asele <- format(asele, big.mark=" ")



df_temp <- filter(df, substr(workplace_address.municipality_code,1,4) == 2403)
bjurholm <- sum(df_temp$number_of_vacancies, na.rm = T)
bjurholm <- format(bjurholm, big.mark=" ")



df_temp <- filter(df, substr(workplace_address.municipality_code,1,4) == 2460)
vannas <- sum(df_temp$number_of_vacancies, na.rm = T)
vannas <- format(vannas, big.mark=" ")


df_temp <- filter(df, substr(workplace_address.municipality_code,1,4) == 2401)
nordmaling <- sum(df_temp$number_of_vacancies, na.rm = T)
nordmaling <- format(nordmaling, big.mark=" ")
```

* `r summa` lediga tjänster i `r geografi` hos Platsbanken, varav:
  + `r sorsele` i Sorsele kommun
  + `r storuman` i Storuman kommun
  + `r skelleftea` i Skellefteå kommun
  + `r mala` i Malå kommun
  + `r norsjo` i Norsjö kommun
  + `r vilhelmina` i Vilhelmina kommun
  + `r lycksele` i Lycksele kommun
  + `r vindeln` i Vindeln kommun
  + `r dorotea` i Dorotea kommun
  + `r robertsfors` i Robertsfors kommun
  + `r umea` i Umeå kommun
  + `r asele` i Åsele kommun
  + `r bjurholm` i Bjurholm kommun
  + `r vannas` i Vannäs kommun
  + `r nordmaling` i Umeå kommun
...
df_22_kom <- df_22_kom %>%
  mutate(occupation_field.label = forcats::fct_reorder(occupation_field.label, total_vac, .fun = sum),
         workplace_address.municipality_code = factor(workplace_address.municipality_code, 
                                                      levels = c("2422","2421","2482","2418","2417","2462","2481","2404","2425","2409","2480","2463","2403","2460","2401")))

color_mapping <- c("2422" = "#800000","2421" = "#9A6324","2482"="#808000","2418"="#469990","2417"="#000075","2462"="#E6194B","2481"="#F58231","2404"="#FFE119","2425"="#3CB44B","2409"="#4363D8","2480"="#911EB4","2463"="#F032E6","2403"="#A9A9A9","2460"="#FABED4","2401"="#DCBEFF")

# VIZ
## ggplot
ggplot(df_22_kom, aes(x = occupation_field.label, y = total_vac, fill = factor(workplace_address.municipality_code, levels = c("2422","2421","2482","2418","2417","2462","2481","2404","2425","2409","2480","2463","2403","2460","2401")))) +
  geom_col() +
...
```
*Kommunkod: 2422 - Sorsele, 2421 - Storuman, 2482 - Skellefteå, 2418 - Malå, 2417 - Norsjö, 2462 - Vilhelmina, 2481 - Lycksele, 2404 - Vindeln, 2425 - Dorotea, 2409 - Robertsfors, 2480 - Umeå, 2463 - Åsele, 2403 - Bjurholm, 2460 - Vännäs, 2401 - Nordmaling. För att underlätta läsning av diagrammet har 'klossar' införts - varje kloss motsvarar 50 tjänster.*
...
ggplot(df_22_omrade, aes(x = ssyk3_text, y = total_vac, fill = 
                           factor(workplace_address.municipality_code, levels = c("2422","2421","2482","2418","2417","2462","2481","2404","2425","2409","2480","2463","2403","2460","2401")))) +
  geom_col() +
  scale_fill_manual(values=color_mapping, guide = guide_legend(reverse = TRUE)) +
  #scale_fill_viridis(discrete = TRUE, direction = 1, guide = guide_legend(reverse = TRUE)) +
  geom_hline(yintercept = seq(from = 0, to = max_value, by = 50), color = "white") +
...
## Filter on 22
df_22_omrade <- filter(df, occupation_field.label == "Naturvetenskap")
...
ggplot(df_22_ssyk1, aes(x = ssyk1_text, y = total_vac, fill = 
                          factor(workplace_address.municipality_code, levels = c("2422","2421","2482","2418","2417","2462","2481","2404","2425","2409","2480","2463","2403","2460","2401")))) +
  geom_col() +
  scale_fill_manual(values=color_mapping, guide = guide_legend(reverse = TRUE)) +
  geom_hline(yintercept = seq(from = 0, to = max_value, by = 50), color = "white") +
...
ggplot(df_22_ssyk1_2_omrade, aes(x = occupation_field.label, y = total_vac, fill = factor(workplace_address.municipality_code, levels = c("2422","2421","2482","2418","2417","2462","2481","2404","2425","2409","2480","2463","2403","2460","2401")))) +
  geom_col() +
  scale_fill_manual(values=color_mapping, guide = guide_legend(reverse = TRUE)) +
  geom_hline(yintercept = seq(from = 0, to = max_value, by = 50), color = "white") +
  theme_tufte() +
...
Kontakt: anna.norin@regionvasterbotten.se
